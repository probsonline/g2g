...
void get_permutations(int set[], int index, int n){
	...
	if (found) return;	//to break the recursion as soon as the match for the next greater permutation is found
...
int main()
{
    ...
    if (t < 1 || t > 40)
    {
    	cout << "Invalid number for test case input. Enter between 1 to 40 test cases.\n";
    	return 0;
    }
...
//		        cout << "Findng permutations"  << endl;        //Find until what number the permutation is in order 
        ...
        {
        	cout << "invalid input. the number must be between 1 to 100 " << endl;
        }
    }
    return 0;
}
